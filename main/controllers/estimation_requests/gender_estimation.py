...
class GenderEstimationController(GenericEstimationController):
    """
    Controller for /estimation-requests/gender/ URL
    """
    ...
    def __init__(self, flask_web_app, available_services, config):
        ...
        """
        GenericEstimationController.__init__(self, flask_web_app, available_services, config, "ESTIMATION", "GENDER")

        self.exposed_methods += [
            self.estimate_gender_of_face_from_content_base64,
            self.estimate_gender_of_face_from_content_stream,
            self.get_available_services
        ]

        self._init_exposed_methods()

    @route("/estimation-requests/gender/services", methods=['GET'])
    def get_available_services(self):
        """
        Retrieves the services available for gender estimation.
        """
        return jsonify(ImageController.get_available_services(self))

    @route("/estimation-requests/gender/face/base64", methods=['PUT'])
    def estimate_gender_of_face_from_content_base64(self):
        """
        Performs a gender estimation on a file containing an image of a face, in the given location.
    ...
    @route("/estimation-requests/gender/face/stream", methods=['PUT'])
    def estimate_gender_of_face_from_content_stream(self):
        """
        Performs a gender estimation on a file containing an image of a face, in the given location.
